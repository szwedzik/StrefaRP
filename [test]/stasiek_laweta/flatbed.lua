...
AddTextEntry(controllerMessageLoweredCar, "Przełącz wajchę ~INPUT_CONTEXT~ aby podnieść ~y~lawetę.~w~Przyciśnij ~INPUT_DETONATE~ aby ~r~odłączyć ~w~pojazd.")
AddTextEntry(controllerMessageLoweredNoCar, "Przełącz wajchę ~INPUT_CONTEXT~ aby podnieść ~y~lawetę ~w~.~n~Przyciśnij ~INPUT_DETONATE~ aby ~g~zaczepić ~w~pojazd.")
AddTextEntry(controllerMessageRaised, "Przełącz wajchę ~INPUT_CONTEXT~ aby ~y~opuścić ~w~lawetę.")
...
function drawMarker(pos)
    ...
    if dist < 25.0 then
        DrawMarker(1, pos, vector3(0.0, 0.0, 0.0), vector3(0.0, 0.0, 0.0), vector3(1.0, 1.0, 0.1), 0, 255, 0, 150)
        if dist < 1.5 then
            return true
        end
    end
