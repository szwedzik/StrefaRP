...
$(function () {
    window.addEventListener('message', function (event) {
        ...
        switch (event.data.type) {
            ...
            case "reportUpdate":
                if (event.data.info == true) {
                    this.document.getElementById('editReportSuccess').style.display = "block";
                } else {
                    this.document.getElementById('editReportFail').style.display = "block";
                }
                break;
            case "updateFinePenal":
                $('#penalCodes').empty();

                var fines = Object.entries(event.data.info);
                for (const [fine, amount] of fines) {
                    $('#penalCodes').append('<li class="dropdown-item" onclick="setPenalActive('+amount+')">'+fine+'</li>');
                  }
                ...
                case "fineIssued":
                    if(event.data.info == true){
                        this.document.getElementById('fineIssued').style.display = "block";
                    }else{
                        this.document.getElementById('fineNotOnline').style.display = "block";
                    }
            default:
                break;
        }
...
function issueFine() {
    issueFineElemnents();
    $.post('http://crumblecad/getFinePenal')
}

function setPenalActive(amount){
var currentAmount = parseInt(document.getElementById('fineAmount').value, 10);
var newAmount = currentAmount + amount;
document.getElementById('fineAmount').value = '';
document.getElementById('fineAmount').value = newAmount;
}

function submitFine(){
var amount = document.getElementById('fineAmount').value;
var reason = document.getElementById('fineIssueReason').value;
$.post('http://crumblecad/submitFine', JSON.stringify({
    amount,
    reason,
    identifier: this.profile.identifier,
    name: this.profile.name,
...
//This will show the edit report page and then prepend a button with the report details.
//These details are then sent to submitEditedReport function below when they click the edit report button.
...
function submitEditedReport(officer, report, date) {
    var newReport = document.getElementById('editReportInput').value;

    $.post('http://crumblecad/editReport', JSON.stringify({
        officer,
        report,
        date,
        newReport
...
function vehicleResults(data, model, color) {
    ...
    $("#vehicleBody").empty(); //empty table on new search
...
function homepageBack() {
    ...
    document.getElementById("issueFine").style.display = "none";
}

function issueFineBack() {
    document.getElementById("issueFine").style.display = "none";
    document.getElementById("citizenResults").style.display = "block";
}
...
function issueFineElemnents(){
    ...
    document.getElementById("citizenReportResults").style.display = "none";
    document.getElementById("convictionResults").style.display = "none";
    document.getElementById("issueFine").style.display = "block";
}
