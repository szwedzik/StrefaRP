...
local locations = {
	[1] = { locked = false, range = 2, outside = { x = 545.55, y = -188.45, z = 54.01, heading = 270.01}, inside = {x = 545.55,y = -188.45,z = 54.01, heading = 270.01}},
...
Citizen.CreateThread(function()
	WarMenu.CreateMenu('LSC', 'Janush Customs')
	WarMenu.SetSubTitle('LSC', 'Janush Customs')
	WarMenu.CreateSubMenu('tunings', 'LSC', 'Tuning Wizualny')
	WarMenu.CreateSubMenu('performance', 'LSC', 'Tuning Osiągów')
	WarMenu.CreateSubMenu('closeMenu', 'LSC', 'Czy aby na pewno?')
	...
	for i,theItem in pairs(vehicleMods) do
		...
		if theItem.id == "paint" then
			WarMenu.CreateSubMenu("primary", theItem.id, "Pierwszy Kolor")
			WarMenu.CreateSubMenu("secondary", theItem.id, "Drugi Kolor")
			
			WarMenu.CreateSubMenu("rimpaint", theItem.id, "Kolor Felg")
			
			WarMenu.CreateSubMenu("classic1", "primary", "Klasyczny Kolor")
			WarMenu.CreateSubMenu("metallic1", "primary", "Metaliczny Kolor")
			WarMenu.CreateSubMenu("matte1", "primary","Matowy Kolor")
			WarMenu.CreateSubMenu("metal1", "primary","Metalowy Kolor")
			WarMenu.CreateSubMenu("classic2", "secondary", "Klasyczny Kolor")
			WarMenu.CreateSubMenu("metallic2", "secondary", "Metaliczny Kolor")
			WarMenu.CreateSubMenu("matte2", "secondary","Matowy Kolor")
			WarMenu.CreateSubMenu("metal2", "secondary","Metalowy Kolor")
			
			WarMenu.CreateSubMenu("classic3", "rimpaint", "Klasyczny Kolor")
			WarMenu.CreateSubMenu("metallic3", "rimpaint", "Metaliczny Kolor")
			WarMenu.CreateSubMenu("matte3", "rimpaint","Matowy Kolor")
			WarMenu.CreateSubMenu("metal3", "rimpaint","Metalowy Kolor")
	...
	while true do
		...
		if WarMenu.IsMenuOpened('LSC') then
			if not IsVehicleDamaged(veh) and WarMenu.MenuButton("Tuning Wizualny", "tunings") then
			elseif not IsVehicleDamaged(veh) and WarMenu.MenuButton("Tuning Osiągów", "performance") then 
			elseif IsVehicleDamaged(veh) and WarMenu.Button('Napraw Pojazd', modPrices.repair.."$") then
				--SetVehicleFixed(veh)
			...
			elseif WarMenu.MenuButton('Wyjdz', 'closeMenu') then
			end
		...
		elseif WarMenu.IsMenuOpened('closeMenu') then
			...
			elseif WarMenu.MenuButton('Nie', 'LSC') then
			end
			WarMenu.Display()
		elseif WarMenu.IsMenuOpened('tunings') then
			...
			WarMenu.Display()
		elseif IsControlJustReleased(0, 244) and veh and GetPedInVehicleSeat(veh, -1) == ped then 
			if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), 545.55, -188.45, 54.01, true) <= 1.2 then --M by default
			--SetVehicleModKit(veh,0)
				SetVehicleInGarage()
				--WarMenu.OpenMenu('LSC')
			else
				exports['mythic_notify']:DoCustomHudText('error', 'Nie jesteś na stanowisku roboczym aby otworzyć katalog!', 3500)
			end
