...
AddEventHandler('esx_mdc:addNewCitizenConviction', function(playerJobName, citizenId, convictionData)
	...
	if xCitizen ~= nil then
		...
		if jailLength == nil or jailLength == 0 then
			TriggerEvent('esx_policejob:pay', fee, citizenServerId, crimes, true)
		else
			sentencespan = jailLength
			TriggerEvent('esx_jail:sendToJail', citizenServerId, jailLength * 60)
			TriggerEvent('esx_policejob:pay', fee, citizenServerId, false)
			TriggerEvent('esx_policejob:unrestrain', citizenServerId)
		end
		...
		function(resultUsers)
			...
			if userData == nil then
				...
				if #resultUsers > 1 then
					...
					local notes = MySQL.Sync.fetchAll('SELECT *, DATE_FORMAT(date, "%d/%m/%Y %H:%i:%s") AS date from lspd_mdc_user_notes WHERE userId = @owner ORDER BY date DESC', {
						['owner'] = userData.identifier
					} );
					if notes == nil then motes = {} end
