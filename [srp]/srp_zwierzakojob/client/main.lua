...
function StopNPCJob(cancel)
	...
	if cancel then
		...
		--TriggerServerEvent('srp_zwierzakojob:onNPCJobCompleted')
	end
end
...
function OpenZwierzakoActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'zwierzako_actions', {
		title    = _U('mechanic'),
		align    = 'top-left',
		elements = elements
	}, function(data, menu)
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'spawn_vehicle', {
					...
					if Config.MaxInService == -1 then
						ESX.Game.SpawnVehicle(data.current.value, Config.Zones.VehicleSpawnPoint.Pos, 250.17, function(vehicle)
							local playerPed = PlayerPedId()
							TaskWarpPedIntoVehicle(playerPed, vehicle, -1)
							plate = VehiclePlaqueMaker('Janush')
...
function OpenZwierzakoActionsMenuShop(source)
	ESX.UI.Menu.CloseAll()
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'zwierzako_actions_menu_shop',
		{
        ...
        function(data, menu)
            if data.current.value == 'naprawka' then
                TriggerServerEvent('srp_zwierzakojob:buyrepairkitcheck')
                --ESX.UI.Menu.CloseAll()
            end	
            --menu.close()
		end,
	...
	if Config.EnablePlayerManagement and ESX.PlayerData.job and ESX.PlayerData.job.grade_name ~= 'recrue' then
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mechanic_harvest', {
			...
			if data.current.value == 'gaz_bottle' then
				TriggerServerEvent('srp_zwierzakojob:startHarvest')
			elseif data.current.value == 'fix_tool' then
				TriggerServerEvent('srp_zwierzakojob:startHarvest2')
			elseif data.current.value == 'caro_tool' then
				TriggerServerEvent('srp_zwierzakojob:startHarvest3')
			end
		end, function(data, menu)
			menu.close()
	...
	if Config.EnablePlayerManagement and ESX.PlayerData.job and ESX.PlayerData.job.grade_name ~= 'recrue' then
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mechanic_craft', {
			...
			if data.current.value == 'blow_pipe' then
				TriggerServerEvent('srp_zwierzakojob:startCraft')
			elseif data.current.value == 'fix_kit' then
				TriggerServerEvent('srp_zwierzakojob:startCraft2')
			elseif data.current.value == 'caro_kit' then
				TriggerServerEvent('srp_zwierzakojob:startCraft3')
			end
		end, function(data, menu)
			menu.close()
...
function OpenMobileZwierzakoActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_zwierzako_actions', {
		title    = _U('mechanic'),
		align    = 'top-left',
		elements = {
		...
		elseif data.current.value == 'open_hood_vehicle' then
			...
			if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
				...
				if(VehicleHoods[vehicle] == nil) then
					...
					TriggerServerEvent("srp_zwierzakojob:setVehicleHoodState", vehicle, 1)
					Citizen.Wait(750)
				else
					if(VehicleHoods[vehicle] == 1) then
						SetVehicleDoorShut(vehicle, 4, 0)
						VehicleHoods[vehicle] = 0
						TriggerServerEvent("srp_zwierzakojob:setVehicleHoodState", vehicle, 0)
						Citizen.Wait(750)
		...
		elseif data.current.value == 'object_spawner' then
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_zwierzako_actions_spawn', {
				title    = _U('objects'),
				align    = 'top-left',
				elements = {
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('srp_zwierzakojob:getStockItems', function(items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('srp_zwierzakojob:getStockItem', itemName, count)
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('srp_zwierzakojob:getPlayerInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('srp_zwierzakojob:putStockItems', itemName, count)
...
function  OpenTakeToolsMenu()
    ...
    if data.current.value == 'water' then
        TriggerServerEvent('srp_zwierzakojob:get_item', data.current.label, data.current.value, 10)
	elseif data.current.type == 'item' then
      TriggerServerEvent('srp_zwierzakojob:get_item', data.current.label, data.current.value, 1)
    elseif data.current.type == 'weapon' then
      TriggerServerEvent('srp_zwierzakojob:giveWeapon', data.current.value)
    end
	end, function(data, menu)
		menu.close()
	end)
...
RegisterNetEvent('srp_zwierzakojob:onHijack')
AddEventHandler('srp_zwierzakojob:onHijack', function()
	local playerPed = GetPlayerPed(-1)
	local coords    = GetEntityCoords(playerPed)
    	...
    	local alarm    = math.random(100)
    		...
    		if alarm <= 33 then
        		...
        		StartVehicleAlarm(vehicle)
	 			if PlayerData.job.name ~= 'zwierzako' and PlayerData.job.name  ~= 'kinia' and PlayerData.job.name  ~= 'police' and PlayerData.job.name  ~= 'sheriff' and PlayerData.job.name  ~= 'statepolice' then
      		...
      		Citizen.CreateThread(function()
        		...
        		Citizen.Wait(30000)
				local jobChance = 30
				if PlayerData.job.name == 'zwierzako' and PlayerData.job.name == 'kinia' or PlayerData.job.name == 'police' and PlayerData.job.name  ~= 'sheriff' and PlayerData.job.name  ~= 'statepolice' then
					jobChance = 100
				end
...
RegisterNetEvent('srp_zwierzakojob:onCarokit')
AddEventHandler('srp_zwierzakojob:onCarokit', function()
	local playerPed = PlayerPedId()
	local coords    = GetEntityCoords(playerPed)
...
RegisterNetEvent('srp_zwierzakojob:onFixkit')
AddEventHandler('srp_zwierzakojob:onFixkit', function()
	local playerPed = PlayerPedId()
	local coords    = GetEntityCoords(playerPed)
	...
	if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
		...
		if DoesEntityExist(vehicle) then
			...
			Citizen.CreateThread(function()
				...
				TriggerServerEvent("srp_zwierzakojob:removeFixTool")
...
RegisterNetEvent('srp_zwierzakojob:onPolFixkit')
AddEventHandler('srp_zwierzakojob:onPolFixkit', function()
	local playerPed = PlayerPedId()
	local coords    = GetEntityCoords(playerPed)
	...
	if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
		...
		if DoesEntityExist(vehicle) then
			...
			Citizen.CreateThread(function()
				exports['mythic_notify']:DoCustomHudText('inform', 'Użyłeś zestawu naprawczego', 3000)
				print("Zaczynam usuwanie")
				TriggerServerEvent("srp_zwierzakojob:removePolFixTool")
				print("Powinno usunąć")
				TriggerEvent("esx_anims:repair", true)
				exports['mythic_progbar']:Progress({
					name = "firstaid_action",
...
AddEventHandler('srp_zwierzakojob:hasEnteredMarker', function(zone)
	if zone == 'NPCJobTargetTowable' then

	elseif zone =='VehicleDelivery' then
		NPCTargetDeleterZone = true
	elseif zone == 'ZwierzakoActions' then
		CurrentAction     = 'zwierzako_actions_menu'
		CurrentActionMsg  = _U('open_actions')
		CurrentActionData = {}
	elseif zone == 'ZwierzakoActionsShop' then
		CurrentAction     = 'zwierzako_actions_menu_shop'
...
AddEventHandler('srp_zwierzakojob:hasExitedMarker', function(zone)
	if zone =='VehicleDelivery' then
		NPCTargetDeleterZone = false
	elseif zone == 'Craft' then
		TriggerServerEvent('srp_zwierzakojob:stopCraft')
		TriggerServerEvent('srp_zwierzakojob:stopCraft2')
		TriggerServerEvent('srp_zwierzakojob:stopCraft3')
	elseif zone == 'Garage' then
		TriggerServerEvent('srp_zwierzakojob:stopHarvest')
		TriggerServerEvent('srp_zwierzakojob:stopHarvest2')
		TriggerServerEvent('srp_zwierzakojob:stopHarvest3')
	end
...
AddEventHandler('srp_zwierzakojob:hasEnteredEntityZone', function(entity)
	local playerPed = PlayerPedId()

	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'zwierzako' and not IsPedInAnyVehicle(playerPed, false) then
...
AddEventHandler('srp_zwierzakojob:hasExitedEntityZone', function(entity)
...
Citizen.CreateThread(function()
	...
	if PlayerData.job and PlayerData.job.name == 'zwierzako' then
	local blipshop = AddBlipForCoord(Config.Zones.ZwierzakoActionsShop.Pos.x, Config.Zones.ZwierzakoActionsShop.Pos.y, Config.Zones.ZwierzakoActionsShop.Pos.z)
...
Citizen.CreateThread(function()
	while true do
		...
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'zwierzako' then
	...
	while true do
		...
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'zwierzako' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('srp_zwierzakojob:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('srp_zwierzakojob:hasExitedMarker', LastZone)
			end
	...
	while true do
		...
		if closestDistance ~= -1 and closestDistance <= 3.0 then
			if LastEntity ~= closestEntity then
				TriggerEvent('srp_zwierzakojob:hasEnteredEntityZone', closestEntity)
				LastEntity = closestEntity
			end
		else
			if LastEntity then
				TriggerEvent('srp_zwierzakojob:hasExitedEntityZone', LastEntity)
				LastEntity = nil
			end
		end
	end
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'zwierzako' then

				if CurrentAction == 'zwierzako_actions_menu' then
					OpenZwierzakoActionsMenu()
				elseif CurrentAction == 'zwierzako_actions_menu_shop' then
					OpenZwierzakoActionsMenuShop()
				elseif CurrentAction == 'zwierzako_harvest_menu' then
					OpenZwierzakoHarvestMenu()
				elseif CurrentAction == 'zwierzako_craft_menu' then
		...
		if IsControlJustReleased(0, Keys['F6']) and not isDead and ESX.PlayerData.job and ESX.PlayerData.job.name == 'zwierzako' then
			OpenMobileZwierzakoActionsMenu()
		end

		--[[
		if IsControlJustReleased(0, 178) and not isDead and ESX.PlayerData.job and ESX.PlayerData.job.name == 'zwierzako' then
			if NPCOnJob then
...
RegisterCommand("odholuj",function(source, args)
    if  ESX.PlayerData.job ~= nil and (ESX.PlayerData.job.name == 'police' or ESX.PlayerData.job.name == 'zwierzako' or ESX.PlayerData.job.name == 'kiniajob' or ESX.PlayerData.job.name == 'sheriff' or ESX.PlayerData.job.name == 'statepolice') then
        local coords    = GetEntityCoords(PlayerPedId())
