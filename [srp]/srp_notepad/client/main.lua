...
RegisterNUICallback('escape', function(data, cb)
    ...
    TriggerEvent("srp_notepad:CloseNotepad")
end)
...
RegisterNetEvent("srp_notepad:OpenNotepadGui")
AddEventHandler("srp_notepad:OpenNotepadGui", function()
    if not isUiOpen then
        openGui()
    end
end)

RegisterNetEvent("srp_notepad:CloseNotepad")
AddEventHandler("srp_notepad:CloseNotepad", function()
    SendNUIMessage({
        action = 'closeNotepad'
    })
...
RegisterNetEvent('srp_notepad:note')
AddEventHandler('srp_notepad:note', function()
    ...
    if ( DoesEntityExist( player ) and not IsEntityDead( player )) then 
        ...
        if ( IsEntityPlayingAnim( player, ad, "base", 3 ) ) then 
            ...
            AttachEntityToEntity(prop, player, GetPedBoneIndex(player, 18905), 0.1, 0.02, 0.05, 10.0, 0.0, 0.0, true, true, false, true, 1, true) -- srp_notepadpad
...
RegisterNetEvent('srp_notepad:updateNotes')
AddEventHandler('srp_notepad:updateNotes', function(serverNotesPassed)
