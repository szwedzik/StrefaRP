$(document).ready(function(){
  ...
  $('input').change(function(){
    $.post('http://srp_character/updateSkin', JSON.stringify({
      value: false,
      // Face
  ...
  $(document).keypress(function(e) {
    if(e.which == 97){ // A pressed
      $.post('http://srp_character/rotaterightheading', JSON.stringify({
        value: 10
      }));
    }
    if(e.which == 101){ // E pressed
      $.post('http://srp_character/rotateleftheading', JSON.stringify({
        value: 10
      }));
    }
  });
  ...
  $('.tab a').on('click', function(e){
    e.preventDefault();
    $.post('http://srp_character/zoom', JSON.stringify({
  ...
  $(document).keypress(function(e){
    ...
    xTriggered++;
    if(e.which == 13){
      $.post('http://srp_character/test', JSON.stringify({
        value: xTriggered
      }));
    }
  });
