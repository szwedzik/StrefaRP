Config                            = {}

Config.DrawDistance               = 100.0
Config.MarkerType                 = 1
Config.MarkerSize                 = { x = 1.5, y = 1.5, z = 0.5 }
Config.MarkerColor                = { r = 50, g = 50, b = 204 }

Config.EnablePlayerManagement3     = true

Config.EnableArmoryManagement3     = true
Config.EnableESXIdentity          = true -- enable if you're using esx_identity
Config.EnableNonFreemodePeds3      = false -- turn this on if you want custom peds
Config.EnableLicenses             = true -- enable if you're using esx_license

Config.EnableHandcuffTimer3        = true -- enable handcuff timer? will unrestrain player after the time ends
Config.HandcuffTimer3              = 25 * 60000 -- 10 mins

Config.EnableJobBlip              = true -- enable blips for colleagues, requires esx_society

Config.MaxInService3               = -1
Config.Locale                     = 'pl'

Config.PoliceStations3 = {
	
	SASP = {

		Blip = { 
			Coords  = vector3(827.11, -1289.97, 28.24),
			Sprite  = 60,
			Display = 4,
			Scale   = 1.2,
			Name	= 'San Andreas State Police',
			Colour  = 3
		},

		Cloakrooms = {
			vector3(832.18, -1294.51, 28.21)
		},

		Armories = {
			vector3(829.53, -1286.47, 28.21)
		},

		Vehicles = {
			{
				Spawner = vector3(855.48, -1285.41, 26.01),
				InsideShop = vector3(228.5, -993.5, -99.5),
				SpawnPoints = {
					{ coords = vector3(854.51, -1277.14, 26.42), heading = 26.42, radius = 6.0 }
				}
			}
		},

		Helicopters = {
			{
				Spawner = vector3(826.52, -1355.28, 26.10),
				InsideShop = vector3(477.0, -1106.4, 43.0),
				SpawnPoints = {
					{ coords = vector3(827.63, -1365.25, 26.51), heading = 270.01, radius = 10.0 }
				}
			}
		},

		BossActions = {
			vector3(836.33, -1291.41, 27.21)
		}

	},
}

Config.AuthorizedWeapons3 = {
	Shared = {
		{ weapon = 'WEAPON_CARBINERIFLE',  components = { nil, nil, nil, 0, 0, nil , 0 ,nil} , price = 0 },
		{ weapon = 'WEAPON_PISTOL', components = { nil, 0, 0, nil, 0}, price = 0 },
		{ weapon = 'WEAPON_COMBATPISTOL', components = { nil, 0, 0, nil, 0}, price = 0 },
		{ weapon = 'WEAPON_SNSPISTOL', components = { nil, 0, 0, nil, nil}, price = 0 },
		{ weapon = 'WEAPON_HEAVYPISTOL', components = { nil, 0, 0, nil, 0}, price = 0 },
		{ weapon = 'WEAPON_PUMPSHOTGUN', price = 0 },
		{ weapon = 'WEAPON_NIGHTSTICK', price = 0 },
		{ weapon = 'WEAPON_STUNGUN', price = 0 },
		{ weapon = 'WEAPON_FLASHLIGHT', price = 0 }
	}
}

Config.Uniforms3 = {
	
	bullet_wear = {
		male = {
			['bproof_1'] = 12,  ['bproof_2'] = 0
		},
		female = {
			['bproof_1'] = 11,  ['bproof_2'] = 0
		}
	},
	gilet_wear = {
		male = {
			['bproof_1'] = 10,  ['bproof_1'] = 0
		},
		female = {
			['bproof_1'] = 36,  ['bproof_1'] = 1
		}
	},
	bullet_wear2 = {
		male = {
			['bproof_1'] = 12,  ['bproof_2'] = 3
		},
		female = {
			['bproof_1'] = 13,  ['bproof_2'] = 1
		}
	},
	handcuffs = {
		male = {
			['chain_1'] = 41,  ['chain_2'] = 0
		},
		female = {
			['chain_1'] = 25,  ['chain_2'] = 0
		}
	},
	no_handcuffs = {
		male = {
			['chain_1'] = 0,  ['chain_2'] = 0
		},
		female = {
			['chain_1'] = 0,  ['chain_2'] = 0
		}
	},
	sasp_wear = {
		male = {
			['tshirt_1'] = 15, ['tshirt_2'] = 0,
			['torso_1'] = 103,   ['torso_2'] = 0,
			['decals_1'] = 0,   ['decals_2'] = 0,
			['chain_1'] = 8, ['chain_2'] = 0,
			['pants_1'] = 32,   ['pants_2'] = 0,
			['shoes_1'] = 33,   ['shoes_2'] = 0,
			['mask_1'] = 0,  ['mask_2'] = 0,
			['helmet_1'] = 17,  ['helmet_2'] = 0,
			['bproof_1'] = 26,  ['bproof_2'] = 2,
			['glasses_1'] = -1,	['arms'] = 17,
			['bags_1'] = 0, ['bags_2'] = 0
		},
		female = {
			['tshirt_1'] = 160,  ['tshirt_2'] = 0,
			['torso_1'] = 94,   ['torso_2'] = 0,
			['decals_1'] = 0,   ['decals_2'] = 0,
			['chain_1'] = 0, ['chain_2'] = 0,
			['pants_1'] = 31,   ['pants_2'] = 0,
			['shoes_1'] = 9,   ['shoes_2'] = 0,
			['mask_1'] = 0,  ['mask_2'] = 0,
			['helmet_1'] = 17,  ['helmet_2'] = 0,
			['bproof_1'] = 28,  ['bproof_2'] = 2,
			['glasses_1'] = -1,	['arms'] = 18,
			['bags_1'] = 0, ['bags_2'] = 0
		}
	},

	sasp2_wear = {
		male = {
			['tshirt_1'] = 38, ['tshirt_2'] = 0,
			['torso_1'] = 190,   ['torso_2'] = 4,
			['decals_1'] = 0,   ['decals_2'] = 0,
			['chain_1'] = 0, ['chain_2'] = 0,
			['pants_1'] = 25,   ['pants_2'] = 1,
			['shoes_1'] = 54,   ['shoes_2'] = 0,
			['mask_1'] = 0,  ['mask_2'] = 0,
			['helmet_1'] = 13,  ['helmet_2'] = 2,
			['bproof_1'] = 13,  ['bproof_2'] = 0,
			['glasses_1'] = 5,	['arms'] = 0,
			['bags_1'] = 55, ['bags_2'] = 0
		},
		female = {
			['tshirt_1'] = 27,  ['tshirt_2'] = 0,
			['torso_1'] = 192,   ['torso_2'] = 4,
			['decals_1'] = 0,   ['decals_2'] = 0,
			['chain_1'] = 0, ['chain_2'] = 0,
			['pants_1'] = 31,   ['pants_2'] = 0,
			['shoes_1'] = 9,   ['shoes_2'] = 0,
			['mask_1'] = 0,  ['mask_2'] = 0,
			['helmet_1'] = 13,  ['helmet_2'] = 2,
			['bproof_1'] = 14,  ['bproof_2'] = 0,
			['glasses_1'] = -1,	['arms'] = 14,
			['bags_1'] = 55, ['bags_2'] = 0
		}
	},

	sasp3_wear = {
		male = {
			['tshirt_1'] = 28, ['tshirt_2'] = 0,
			['torso_1'] = 36,   ['torso_2'] = 5,
			['decals_1'] = 0,   ['decals_2'] = 0,
			['chain_1'] = 6, ['chain_2'] = 0,
			['pants_1'] = 35,   ['pants_2'] = 0,
			['shoes_1'] = 54,   ['shoes_2'] = 0,
			['mask_1'] = 0,  ['mask_2'] = 0,
			['helmet_1'] = 13,  ['helmet_2'] = 2,
			['bproof_1'] = 23,  ['bproof_2'] = 2,
			['glasses_1'] = -1,	['arms'] = 6,
			['bags_1'] = 0, ['bags_2'] = 0
		},
		female = {
			['tshirt_1'] = 38,  ['tshirt_2'] = 0,
			['torso_1'] = 168,   ['torso_2'] = 5,
			['decals_1'] = 0,   ['decals_2'] = 0,
			['chain_1'] = 6, ['chain_2'] = 0,
			['pants_1'] = 31,   ['pants_2'] = 1,
			['shoes_1'] = 9,   ['shoes_2'] = 0,
			['mask_1'] = 0,  ['mask_2'] = 0,
			['helmet_1'] = 13,  ['helmet_2'] = 2,
			['bproof_1'] = 25,  ['bproof_2'] = 2,
			['glasses_1'] = -1,	['arms'] = 7,
			['bags_1'] = 0, ['bags_2'] = 0
		}
	},

	sasp4_wear = {
		male = {
			['tshirt_1'] = 65, ['tshirt_2'] = 6,
			['torso_1'] = 103,   ['torso_2'] = 0,
			['decals_1'] = 0,   ['decals_2'] = 0,
			['chain_1'] = 0, ['chain_2'] = 0,
			['pants_1'] = 32,   ['pants_2'] = 0,
			['shoes_1'] = 33,   ['shoes_2'] = 0,
			['mask_1'] = 0,  ['mask_2'] = 0,
			['helmet_1'] = 13,  ['helmet_2'] = 2,
			['bproof_1'] = 14,  ['bproof_2'] = 0,
			['glasses_1'] = -1,	['arms'] = 17,
			['bags_1'] = 0, ['bags_2'] = 0
		},
		female = {
			['tshirt_1'] = 45,  ['tshirt_2'] = 6,
			['torso_1'] = 94,   ['torso_2'] = 0,
			['decals_1'] = 0,   ['decals_2'] = 0,
			['chain_1'] = 8, ['chain_2'] = 0,
			['pants_1'] = 31,   ['pants_2'] = 0,
			['shoes_1'] = 9,   ['shoes_2'] = 0,
			['mask_1'] = 0,  ['mask_2'] = 0,
			['helmet_1'] = 13,  ['helmet_2'] = 2,
			['bproof_1'] = 16,  ['bproof_2'] = 0,
			['glasses_1'] = -1,	['arms'] = 18,
			['bags_1'] = 0, ['bags_2'] = 0
		}
	},

	sasp5_wear = {
		male = {
			['tshirt_1'] = 53, ['tshirt_2'] = 0,
			['torso_1'] = 200,   ['torso_2'] = 4,
			['decals_1'] = 0,   ['decals_2'] = 0,
			['chain_1'] = 0, ['chain_2'] = 0,
			['pants_1'] = 25,   ['pants_2'] = 1,
			['shoes_1'] = 54,   ['shoes_2'] = 0,
			['mask_1'] = 0,  ['mask_2'] = 0,
			['helmet_1'] = 13,  ['helmet_2'] = 2,
			['bproof_1'] = 13,  ['bproof_2'] = 0,
			['glasses_1'] = -1,	['arms'] = 1,
			['bags_1'] = 55, ['bags_2'] = 0
		},
		female = {
			['tshirt_1'] = 27,  ['tshirt_2'] = 0,
			['torso_1'] = 202,   ['torso_2'] = 4,
			['decals_1'] = 0,   ['decals_2'] = 0,
			['chain_1'] = 0, ['chain_2'] = 0,
			['pants_1'] = 31,   ['pants_2'] = 0,
			['shoes_1'] = 9,   ['shoes_2'] = 0,
			['mask_1'] = 0,  ['mask_2'] = 0,
			['helmet_1'] = 13,  ['helmet_2'] = 2,
			['bproof_1'] = 14,  ['bproof_2'] = 0,
			['glasses_1'] = -1,	['arms'] = 3,
			['bags_1'] = 55, ['bags_2'] = 0
		}
	},

	saspkadet_wear = {
		male = {
			['tshirt_1'] = 53, ['tshirt_2'] = 0,
			['torso_1'] = 200,   ['torso_2'] = 4,
			['decals_1'] = 0,   ['decals_2'] = 0,
			['chain_1'] = 0, ['chain_2'] = 0,
			['pants_1'] = 25,   ['pants_2'] = 1,
			['shoes_1'] = 54,   ['shoes_2'] = 0,
			['mask_1'] = 0,  ['mask_2'] = 0,
			['helmet_1'] = 13,  ['helmet_2'] = 2,
			['bproof_1'] = 10,  ['bproof_2'] = 2,
			['glasses_1'] = -1,	['arms'] = 1,
			['bags_1'] = 55, ['bags_2'] = 0
		},
		female = {
			['tshirt_1'] = 27,  ['tshirt_2'] = 0,
			['torso_1'] = 202,   ['torso_2'] = 4,
			['decals_1'] = 0,   ['decals_2'] = 0,
			['chain_1'] = 0, ['chain_2'] = 0,
			['pants_1'] = 31,   ['pants_2'] = 0,
			['shoes_1'] = 9,   ['shoes_2'] = 0,
			['mask_1'] = 0,  ['mask_2'] = 0,
			['helmet_1'] = 13,  ['helmet_2'] = 2,
			['bproof_1'] = 19,  ['bproof_2'] = 2,
			['glasses_1'] = -1,	['arms'] = 3,
			['bags_1'] = 55, ['bags_2'] = 0
		}
	},

	sasp6_wear = {
		male = {
			['tshirt_1'] = 65, ['tshirt_2'] = 2,
			['torso_1'] = 189,   ['torso_2'] = 9,
			['decals_1'] = 0,   ['decals_2'] = 0,
			['chain_1'] = 0, ['chain_2'] = 0,
			['pants_1'] = 25,   ['pants_2'] = 1,
			['shoes_1'] = 54,   ['shoes_2'] = 0,
			['mask_1'] = 0,  ['mask_2'] = 0,
			['helmet_1'] = 44,  ['helmet_2'] = 0,
			['bproof_1'] = 23,  ['bproof_2'] = 2,
			['glasses_1'] = -1,	['arms'] = 3,
			['bags_1'] = 55, ['bags_2'] = 0
		},
		female = {
			['tshirt_1'] = 45,  ['tshirt_2'] = 2,
			['torso_1'] = 191,   ['torso_2'] = 9,
			['decals_1'] = 0,   ['decals_2'] = 0,
			['chain_1'] = 0, ['chain_2'] = 0,
			['pants_1'] = 31,   ['pants_2'] = 0,
			['shoes_1'] = 52,   ['shoes_2'] = 0,
			['mask_1'] = 0,  ['mask_2'] = 0,
			['helmet_1'] = 43,  ['helmet_2'] = 0,
			['bproof_1'] = 25,  ['bproof_2'] = 2,
			['glasses_1'] = -1,	['arms'] = 3,
			['bags_1'] = 55, ['bags_2'] = 0
		}
	},

	sasp7_wear = {
		male = {
			['tshirt_1'] = 15, ['tshirt_2'] = 0,
			['torso_1'] = 53,   ['torso_2'] = 1,
			['decals_1'] = 9,   ['decals_2'] = 0,
			['chain_1'] = 1, ['chain_2'] = 0,
			['pants_1'] = 37,   ['pants_2'] = 1,
			['shoes_1'] = 25,   ['shoes_2'] = 0,
			['mask_1'] = 56,  ['mask_2'] = 3,
			['helmet_1'] = 39,  ['helmet_2'] = 1,
			['bproof_1'] = 27,  ['bproof_2'] = 2,
			['glasses_1'] = -1,	['arms'] = 17,
			['bags_1'] = 55, ['bags_2'] = 0
		},
		female = {
			['tshirt_1'] = 15,  ['tshirt_2'] = 0,
			['torso_1'] = 46,   ['torso_2'] = 1,
			['decals_1'] = 8,   ['decals_2'] = 0,
			['chain_1'] = 1, ['chain_2'] = 0,
			['pants_1'] = 36,   ['pants_2'] = 1,
			['shoes_1'] = 25,   ['shoes_2'] = 0,
			['mask_1'] = 56,  ['mask_2'] = 3,
			['helmet_1'] = 38,  ['helmet_2'] = 1,
			['bproof_1'] = 29,  ['bproof_2'] = 2,
			['glasses_1'] = -1,	['arms'] = 17,
			['bags_1'] = 55, ['bags_2'] = 0
		}
	},

	sasp8_wear = {
		male = {
			['tshirt_1'] = 38, ['tshirt_2'] = 0,
			['torso_1'] = 51,   ['torso_2'] = 1,
			['decals_1'] = 0,   ['decals_2'] = 0,
			['chain_1'] = 1, ['chain_2'] = 0,
			['pants_1'] = 33,   ['pants_2'] = 0,
			['shoes_1'] = 25,   ['shoes_2'] = 0,
			['mask_1'] = 0,  ['mask_2'] = 0,
			['helmet_1'] = 44,  ['helmet_2'] = 0,
			['bproof_1'] = 13,  ['bproof_2'] = 0,
			['glasses_1'] = -1,	['arms'] = 0,
			['bags_1'] = 0, ['bags_2'] = 0
		},
		female = {
			['tshirt_1'] = 27,  ['tshirt_2'] = 1,
			['torso_1'] = 44,   ['torso_2'] = 1,
			['decals_1'] = 0,   ['decals_2'] = 0,
			['chain_1'] = 0, ['chain_2'] = 0,
			['pants_1'] = 54,   ['pants_2'] = 3,
			['shoes_1'] = 25,   ['shoes_2'] = 0,
			['mask_1'] = 0,  ['mask_2'] = 0,
			['helmet_1'] = 43,  ['helmet_2'] = 0,
			['bproof_1'] = 14,  ['bproof_2'] = 0,
			['glasses_1'] = -1,	['arms'] = 14,
			['bags_1'] = 0, ['bags_2'] = 0
		}
	},
}
