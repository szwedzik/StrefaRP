...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if pedInVeh or locationAlwaysOn then
            ...
            drawTxt(timeText, 4, locationColorText, 0.4, screenPosX, screenPosY + 0.018)
            ...
            if pedInVeh and GetIsVehicleEngineRunning(vehicle) and vehicleClass ~= 13 then
                ...
                if ShouldUseMetricMeasurements() then
                    ...
                    drawTxt(("%.3d"):format(math.ceil(speed)), 2, speedColor, 0.5, screenPosX + 0.000, screenPosY + 0.044)
                    drawTxt("KMH", 2, speedColorText, 0.25, screenPosX + 0.020, screenPosY + 0.048)
                end

                -- Draw seatbelt status if not a motorcyle
                if vehicleClass ~= 8 then
                    local seatbeltColor = seatbeltIsOn and seatbeltColorOn or seatbeltColorOff
                    drawTxt("Pasy", 2, seatbeltColor, 0.5, screenPosX + 0.045, screenPosY + 0.044)
                end
            end
        end
    end
