...
-- Script starts here
local helicam = false
local polmav_hash = GetHashKey("vcpdmav11")
local sasp_hash = GetHashKey("vcpdmav12")
local sheriff_hash = GetHashKey("vcpdmav21")
local ambulance_hash = GetHashKey("vcpdmav22")
local fov = (fov_max+fov_min)*0.5
local vision_state = 0 -- 0 is normal, 1 is nightmode, 2 is thermal vision
Citizen.CreateThread(function()
	while true do
        Citizen.Wait(10)
		if IsPlayerInPolmav() or IsPlayerInsasp() or IsPlayerInsheriff() or IsPlayerInAmbulance() then
			...
			if IsHeliHighEnough(heli) then
				...
				if IsControlJustPressed(0, toggle_rappel) then -- Initiate rappel
					...
					if GetPedInVehicleSeat(heli, 1) == lPed or GetPedInVehicleSeat(heli, 2) == lPed then
						...
						AddTextComponentString("~r~Nie zrobisz tego z przedniego fotela.")
...
function IsPlayerInsasp()
	local lPed = GetPlayerPed(-1)
	local vehicle = GetVehiclePedIsIn(lPed)
	return IsVehicleModel(vehicle, sasp_hash)
end

function IsPlayerInsheriff()
	local lPed = GetPlayerPed(-1)
	local vehicle = GetVehiclePedIsIn(lPed)
	return IsVehicleModel(vehicle, sheriff_hash)
end

function IsPlayerInAmbulance()
	local lPed = GetPlayerPed(-1)
	local vehicle = GetVehiclePedIsIn(lPed)
	return IsVehicleModel(vehicle, ambulance_hash)
end
...
function RenderVehicleInfo(vehicle)
	...
	SetTextOutline()
	SetTextEntry("STRING")
	AddTextComponentString("Pojazd: "..vehname.."\nTablice: "..licenseplate)
	DrawText(0.45, 0.9)
end
