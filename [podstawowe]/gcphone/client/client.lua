...
function showFixePhoneHelper (coords)
  for number, data in pairs(FixePhone) do
    local dist = GetDistanceBetweenCoords(coords.x, coords.y, coords.z, data.coords.x, data.coords.y, data.coords.z)
    if dist <= 1 then
      DrawText3D(data.coords.x, data.coords.y, data.coords.z+1.1, "~g~[E]~w~ Aby użyć budki telefonicznej~n~Numer budki: "..number)
...
Citizen.CreateThread(function ()
  ...
  while true do
    ...
    for i, _ in pairs(PhoneInCall) do 
        local dist = GetDistanceBetweenCoords(coords.x, coords.y, coords.z, PhoneInCall[i].coords.x, PhoneInCall[i].coords.y, PhoneInCall[i].coords.z)
        if (dist <= soundDistanceMax) then
          DrawText3D(PhoneInCall[i].coords.x, PhoneInCall[i].coords.y, PhoneInCall[i].coords.z+1.1, "~g~[E]~w~ Aby odebrac")
    ...
    if ShowNumberNotification == true then
      ...
      for _,contact in pairs(contacts) do
        if contact.number == message.transmitter then
          ...
          exports['mythic_notify']:DoHudText('inform', "Otrzymałeś wiadomość od: "..contact.display.."", 10000)
          --break
        elseif not contact.number == message.transmitter then
          exports['mythic_notify']:DoHudText('inform', "Otrzymałeś wiadomość od: "..message.transmitter.."", 10000)
        end
      end
    end
