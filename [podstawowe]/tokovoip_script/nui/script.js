...
function checkPluginVersion() {
	if (isPluginVersionCorrect()) {
		document.getElementById('pluginVersion').innerHTML = `<font size="3px">Wersja wtyczki</font><br> <font color="green">${voip.pluginVersion}</font>`;
	} else {
		document.getElementById('pluginVersion').innerHTML = `<font size="3px">Wersja wtyczki</font><br> <font color="red">${voip.pluginVersion}</font> (Wymagana: ${voip.minVersion})`;
...
function updateTokovoipInfo(msg) {
	...
	switch (voipStatus) {
		case NOT_CONNECTED:
			msg = 'Nie jesteś połączony z naszym serwerem TeamSpeak.';
			color = 'red';
			break;
		case PLUGIN_INITIALIZING:
			msg = 'Trwa inicjowanie VoiceChatu...';
			color = 'red';
			break;
		case WRONG_SERVER:
			msg = `Connected to the wrong TeamSpeak server, please join the server: <font color="#e17724">${voip.plugin_data.TSServer}</font>`;
			screenMessage = `Jesteś połączony z złym serwerem TeamSpeak. <br>Prosimy abyś połączym się z <br>serwerem: <font color="#e17724">${voip.plugin_data.TSServer}</font>`;
			color = 'red';
			break;
		case WRONG_CHANNEL:
			msg = `Connected to the wrong TeamSpeak channel, please join the channel: <font color="#e17724">${voip.plugin_data.TSChannelWait && voip.plugin_data.TSChannelWait !== '' && voip.plugin_data.TSChannelWait || voip.plugin_data.TSChannel}</font>`;
			screenMessage = `Znajdujesz się na złym kanale! <br>Prosimy abyś połączył się z <br>kanałem: <font color="#e17724">${voip.plugin_data.TSChannelWait && voip.plugin_data.TSChannelWait !== '' && voip.plugin_data.TSChannelWait || voip.plugin_data.TSChannel}</font>`;
			color = 'red';
			break;
		case INCORRECT_VERSION:
			msg = 'Zła wersja pluginu!';
			screenMessage = 'Zła wersja pluginu!';
			color = 'red';
			break;
		case OK:
			color = '#e17724';
	...
	document.getElementById('pluginStatus').innerHTML = `<font size="3px">Status</font><br> <b><font color="${color}">${screenMessage || msg}</font></b>`;
}

function updateConfig(payload) {
	voip = payload;
	//document.getElementById('pname').innerHTML = `<div id="pname" style="font-size:20px;color:#fff;text-align:center;margin-bottom:20px;">Witaj, ${voip.plugin_data.playername} !</div>`;
	document.getElementById('TSServer').innerHTML = `<font size="3px">IP serwera</font><br> <font color="#e17724">${voip.plugin_data.TSServer}</font>`;
	document.getElementById('TSChannel').innerHTML = `<font size="3px">Domyślny kanał</font><br> <font color="#e17724">${(voip.plugin_data.TSChannelWait) ? voip.plugin_data.TSChannelWait : voip.plugin_data.TSChannel}</font>`;
