	...
	Citizen.CreateThread(function()
		while (true) do
			...
			elseif (IsControlJustPressed(0, self.keyProximity)) then -- Switch proximity modes (normal / whisper / shout)
				...
				local voipinfo = "";
				if (self.mode == 1) then
					voipinfo = "Od teraz mówisz <b>szeptem</b>";
				elseif (self.mode == 2) then
					voipinfo = "Od teraz mówisz <b>normalnie</b>";
				elseif (self.mode == 3) then
					voipinfo = "Od teraz <b>krzyczysz</b>";
				end
				setPlayerData(self.serverId, "voip:mode", self.mode, true);
				self:updateTokoVoipInfo();
				exports['mythic_notify']:DoHudText('inform', voipinfo, 5000)
			end
