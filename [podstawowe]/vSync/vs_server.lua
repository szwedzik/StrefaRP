...
admins = {
    'steam:11000010c93901b', -- Ezi
    'steam:110000107fb8495', -- DaMcHm
    'steam:11000010229c988', -- Buli
    'steam:11000010d5452e1', -- Ciemina
    'steam:1100001054dbe9d', -- Kinia
}
...
RegisterCommand('freezetime', function(source, args)
    if source ~= 0 then
        if isAllowedToChange(source) then
            ...
            TriggerClientEvent('chatMessage', source, '', {255,255,255}, '^8Błąd: ^1Nie masz praw do tego.')
        end
    else
        freezeTime = not freezeTime
        if freezeTime then
    ...
    if source == 0 then
        ...
        if isAllowedToChange(source) then
            ...
            if args[1] == nil then
                ...
                if validWeatherType then
                    TriggerClientEvent('vSync:notify', source, 'Pogoda zmieni się wkrótce na ~y~' .. string.lower(args[1]) .. "~s~.")
                    ...
                    TriggerEvent('srp_logs:adminLog', GetPlayerName(source), "/weather "..args[1], discordid, sourceXPlayer.identifier, licenserc, ip)
                    CurrentWeather = string.upper(args[1])
                    newWeatherTimer = 10
                    TriggerEvent('vSync:requestSync')
                else
    ...
    elseif source ~= 0 then
        if isAllowedToChange(source) then
            if tonumber(args[1]) ~= nil and tonumber(args[2]) ~= nil then
                ...
                TriggerClientEvent('vSync:notify', source, 'Czas został zmieniony na ~y~' .. newtime .. "~s~.")
                local sourceXPlayer = ESX.GetPlayerFromId(source)
                local ip = GetPlayerEndpoint(source)
                local identifier = GetPlayerIdentifiers(source)[1]
                for _, foundID in ipairs(GetPlayerIdentifiers(source)) do
                ...
                TriggerEvent('srp_logs:adminLog', GetPlayerName(source), "/time "..newtime, discordid, sourceXPlayer.identifier, licenserc, ip)
                TriggerEvent('vSync:requestSync')
            else
