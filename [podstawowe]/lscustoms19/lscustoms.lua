...
local garages = { 
	[1]= { locked = false, --[[camera = {x =544.71,y = -188.22, z = 57.08, heading = 270.01},]] driveout = {x = 544.71,y = -188.22, z = 54.08, heading = 270.01}, drivein = {x = 540.18,y = -188.54, z = 54.08, heading = 270.01}, outside = {x = 544.71,y = -188.54, z = 54.08, heading = 270.01}, inside = {x = 544.71,y = -188.22, z = 54.08, heading = 270.01}}
}
local garages2 = { 
	[1]= { locked = false, --[[camera = {x =544.71,y = -188.22, z = 57.08, heading = 270.01},]] driveout = {x = -328.03,y = -144.7, z = 38.62, heading = 251.01}, drivein = {x = -332.69,y = -142.68, z = 38.65, heading = 251.01}, outside = {x = -328.03,y = -144.7, z = 38.62, heading = 251.01}, inside = {x = -328.03,y = -144.7, z = 38.62, heading = 251.01}}
-- driveout - środek
-- drivein - przed
-- outside - środek
-- inside - środek
...
--Setup main menu
local LSCMenu = Menu.new("Janush Customs","CATEGORIES", 0.16,0.13,0.24,0.36,0,{255,255,255,255})
...
local function DriveInGarage()
	...
	if DoesEntityExist(veh) then
		...
		if currentgarage == 4 or currentgarage == 5 then
			LSCMenu:setTitle("Janush Customs")
		...
		SetIbuttons({
			{GetControlInstructionalButton(1,LSCMenu.config.controls.menu_back, 0),"Cofnij"},
			{GetControlInstructionalButton(1,LSCMenu.config.controls.menu_select, 0),"Wybierz"},
			{GetControlInstructionalButton(1,LSCMenu.config.controls.menu_up, 0),"W górę"},
			{GetControlInstructionalButton(1,LSCMenu.config.controls.menu_down, 0),"W dół"},
			{GetControlInstructionalButton(1,LSCMenu.config.controls.menu_left, 0),"W lewo"},
			{GetControlInstructionalButton(1,LSCMenu.config.controls.menu_right, 0),"W prawo"},
		},0)
		...
		Citizen.CreateThread(function()
			...
			TaskVehicleDriveToCoord(ped, veh, pos.inside.x+1.5, pos.inside.y, pos.inside.z, f(2), f(1), GetEntityModel(veh), 16777216, f(0.1), true)
...
Citizen.CreateThread(function()
	while true do
		...
		if PlayerData.job ~= nil then
			if PlayerData.job.name == 'zwierzako' then
								...
								if LSC_Config.oldenter then			
									...
									if GetDistanceBetweenCoords(outside.x,outside.y,outside.z,GetEntityCoords(ped)) <= f(5) then
										...
										if not tableContains(LSC_Config.ModelBlacklist,GetDisplayNameFromVehicleModel(GetEntityModel(veh)):lower()) then
											...
											if pos.locked then
												...
												if not LSC_Config.lock then
													if IsControlJustPressed(1,201) then
														...
														drawTxt("~r~ENTER~w~ aby otworzyć katalog ~r~Janush Customs",4,1,0.5,0.8,0.4,255,255,255,255)
													end
												else
													drawTxt("~r~Zamknięte, prosze poczekać",4,1,0.5,0.8,0.4,255,255,255,255)
												end
											else
												if IsControlJustPressed(1,201) then
													inside = true
											...
											drawTxt("~r~Ten pojazd nie może być ulepszany",4,1,0.5,0.8,0.4,255,255,255,255)
										end
									end
								else
										...
										if not tableContains(LSC_Config.ModelBlacklist,GetDisplayNameFromVehicleModel(GetEntityModel(veh)):lower()) then
											...
											if pos.locked then
												...
												if not LSC_Config.lock then
													...
													drawTxt("~r~Zakmnięte, proszę czekać",4,1,0.5,0.8,1.0,255,255,255,255)
												end
											else
												inside = true
												currentpos = pos
											...
											drawTxt("~r~Ten pojazd nie może być ulepszany",4,1,0.5,0.8,1.0,255,255,255,255)
										end
									end
								end
							end
...
Citizen.CreateThread(function()
	while true do
		...
		--If you are not already in garage
		if PlayerData.job ~= nil then
			if PlayerData.job.name == 'kiniajob' then
				if inside == false then
					local ped = LocalPed()
					--Well... yes... we actually need a car to do something
					if IsPedSittingInAnyVehicle(ped) then
						...
						if DoesEntityExist(veh) and GetPedInVehicleSeat(veh, -1) == ped and (IsThisModelACar(GetEntityModel(veh)) or IsThisModelABike(GetEntityModel(veh)))  then
							--So lets go through every garage
							for i,pos in ipairs(garages2) do
								--Lets take the outside coords of garage
								outside = pos.drivein	
								--Old enter:If vehicle is close enough, then text will be displayed - Press ENTER to enter garage
								if LSC_Config.oldenter then			
											...
											if pos.locked then
												...
												if not LSC_Config.lock then
													if IsControlJustPressed(1,201) then
														...
														drawTxt("~r~ENTER~w~ aby otworzyć katalog ~r~Carzone Garage",4,1,0.5,0.8,0.4,255,255,255,255)
													end
												else
													drawTxt("~r~Zamknięte, prosze poczekać",4,1,0.5,0.8,0.4,255,255,255,255)
												end
...
function CanPurchase(price, canpurchase)
	if canpurchase then
		if LSCMenu.currentmenu == "main" then
			LSCMenu:showNotification("Pojazd został naprawiony.")
		else
			LSCMenu:showNotification("Zakupiono.")
		end
		return true
	else
		LSCMenu:showNotification("~r~Nie masz wystarczająco pieniędzy.")
		return false
	end
end
...
--[[local function AddBlips()
	for i,pos in ipairs(garages) do
		...
		if i == 5 then
			...
			AddTextComponentString("Janush Customs Exclusive")
