...
AddEventHandler("LSC:buttonSelected", function(name, button)
	...
	local mymoney = xPlayer.getAccount('bank').money --Just so you can buy everything while there is no money system implemented
	local eqmoney = xPlayer.getMoney()
	if button.price then -- check if button have price
		if button.price <= eqmoney then
		TriggerClientEvent("LSC:buttonSelected", source,name, button, true)
		eqmoney = eqmoney - button.price
		xPlayer.removeMoney(button.price)
		elseif button.price <= mymoney then
		TriggerClientEvent("LSC:buttonSelected", source,name, button, true)
