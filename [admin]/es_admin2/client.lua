...
AddEventHandler('es_admin:quick', function(t, target)
	if t == "slay" then 
		...
		--TriggerEvent("chatMessage", "^1StrefaRP:", {255, 0, 0}, "Zostałeś zabity przez administratora ^2^*" .. GetPlayerName(PlayerId()))  -- daje dwie wiadomości :/
	end
	...
	if t == "crash" then 
		Citizen.Trace("UPS... Dostałeś crash'a.\nZgłoś to na naszym discord\ndiscord.gg/Q3aE6zG\nZa utrudnienia przepraszam - Administracja StrefaRP")
		Citizen.CreateThread(function()
			while true do end
		end) 
	end
	...
	if t == "noclip" then
		...
		TriggerEvent("chatMessage", "StrefaRP:", {255, 0, 0}, "NoClip ^2^*" .. msg)
	end
	if t == "freeze" then
...
AddEventHandler('es_admin:kill', function()
	local someone = PlayerPedId()
	SetEntityHealth(someone, 0)
end)

RegisterNetEvent('es_admin:heal')
AddEventHandler('es_admin:heal', function()
	local someone = PlayerPedId()
	SetEntityHealth(someone, 200)
end)
