...
AddEventHandler('esx_shops:buyItem', function(itemName, amount, zone)
	...
	if xPlayer.getMoney() >= price then
		...
		if sourceItem.limit ~= -1 and (sourceItem.count + amount) > sourceItem.limit then
			TriggerClientEvent('mythic_notify:client:SendJobAlert', _source, { type = 'error', text = 'Twoje kieszenie są już pełne!' })
		else
			xPlayer.removeMoney(price)
			xPlayer.addInventoryItem(itemName, amount)
			--TriggerClientEvent('esx:showNotification', _source, _U('bought', amount, itemLabel, price))
			if amount > 1 then
				TriggerClientEvent('mythic_notify:client:SendJobAlert', _source, { type = 'inform', text = 'Kupiłeś x' ..amount..' '..itemLabel..' za '..price..'$' })
			else
				TriggerClientEvent('mythic_notify:client:SendJobAlert', _source, { type = 'inform', text = 'Kupiłeś '..itemLabel..' za '..price..'$' }) 
			end
		end
	else
		local missingMoney = price - xPlayer.getMoney()
		--TriggerClientEvent('esx:showNotification', _source, _U('not_enough', missingMoney))
		TriggerClientEvent('mythic_notify:client:SendJobAlert', _source, { type = 'error', text = 'Brakuje Ci '..missingMoney..'$' })
	end
end)