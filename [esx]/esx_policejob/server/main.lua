...
TriggerEvent('esx_phone:registerNumber', 'police', 'Telefon Alarmowy', true, true)
...
AddEventHandler('esx_policejob:jail', function(amount, target, itsJail)
	...
	if itsJail then
		GetRPName(target, function(firstname, lastname, jailReason, jailTime)
			--TriggerClientEvent('chat:addMessage', -1, { args = { "[NEWS]",  "^4" .. firstname .. " " .. lastname .. "^7 otrzymał mandat w wysokości ^4" .. amount .. "$^7 za ^4" .. charge}, color = { 255, 166, 0 } })
			TriggerClientEvent('chat:addMessage', -1, { args = { "[NEWS]",  "^4" .. firstname .. " " .. lastname .. "^7 został skazany na ^4" .. jailTime .. "^7 miesięcy oraz karę grzywny w wysokości^4" .. amount .. "^7za ^4" .. jailReason}, color = { 255, 166, 0 } })
		end)
	end
end)
...
AddEventHandler("policejob:alert", function(drogi)
	TriggerClientEvent("outlawNotify", -1, "[^7Centrala]: ^810-13 ^7 - Ranny Funkcjonariusz^7 | ^3na ulicy ^7" ..drogi)
end)
...
RegisterCommand("bagaznik", function(source,args, raw)
    local src = source
    TriggerClientEvent('zrider:TrunkKidnapping',src)
end)

ESX.RegisterServerCallback('zrider:targetCuffsCallback', function(source, cb, target)
	if HandcuffStatus[target]== nil then
		TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'Porwana osoba musi być zakuta!', lenght=3000 })
		cb(false)
	elseif HandcuffStatus[target]== false then
		...
		if TrunkStatus[target] == nil then
			...
			TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'Wsadzasz '..GetPlayerName(target), lenght=3000 })
		elseif TrunkStatus[target] == false then
			TrunkStatus[target] = true
			TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'Wsadzasz '..GetPlayerName(target), lenght=3000 })
		else
			TrunkStatus[target] = false
			TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'Wyciągasz '..GetPlayerName(target), lenght=3000 })
		end
		cb(true)
	end
end)
