...
local TaxiBlip = vector3(172.25, -26.50, 68.34)
...
function StartTaxiJob()
  ...
  ShowLoadingPromt(myCar, 5000, 3)
  ShowLoadingPromt(_U('taking_service') .. 'Taxi', 5000, 3)
  ClearCurrentMission()
...
function OpenTaxiActionsMenu()
  ...
  if PlayerData.job ~= nil and PlayerData.job.grade > 0 then
    table.insert(elements, {label = _U('spawn_veh'), value = 'spawn_vehicle'})
  end
...
-- Create Blips
Citizen.CreateThread(function()
    bliptaxi = AddBlipForCoord(TaxiBlip)

    SetBlipSprite (bliptaxi, 198)
    SetBlipDisplay(bliptaxi, 4)
    SetBlipScale  (bliptaxi, 0.7)
    SetBlipColour (bliptaxi, 46)
    SetBlipAsShortRange(bliptaxi, true)

    BeginTextCommandSetBlipName("STRING")
    AddTextComponentString("Postój Taksówek")
    ...
    if OnJob then
      ...
      if CurrentCustomer == nil then
        ...
        if IsPedInAnyVehicle(playerPed,  false) and GetEntitySpeed(playerPed) >= 0 then
          ...
          local waitUntil = GetGameTimer() + GetRandomIntInRange(20000,  30000)
          ...
          if OnJob and IsPedInAnyVehicle(playerPed,  false) and GetEntitySpeed(playerPed) >= 0 then
            ...
            if CurrentCustomer ~= nil then
              ...
              local standTime = GetRandomIntInRange(30000,  120000)
          ...
          if IsPedSittingInVehicle(CurrentCustomer,  vehicle) then
            ...
            if CustomerEnteredVehicle then
              ...
              if TargetCoords ~= nil then
                DrawMarker(27, TargetCoords.x, TargetCoords.y, TargetCoords.z - 0.9, 0, 0, 0, 0, 0, 0, 4.0, 4.0, 2.0, 178, 236, 93, 155, 0, 0, 2, 0, 0, 0, 0)
              end
            ...
            DrawMarker(27, customerCoords.x, customerCoords.y, customerCoords.z - 0.9, 0, 0, 0, 0, 0, 0, 4.0, 4.0, 2.0, 178, 236, 93, 155, 0, 0, 2, 0, 0, 0, 0)
...
Citizen.CreateThread(function()
  while true do
    ...
    if CurrentAction ~= nil then
      ...
      if IsControlPressed(0, Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'taxi' and (GetGameTimer() - GUI.Time) > 300 then
        ...
        if CurrentAction == 'delete_vehicle' then
          ...
          if Config.EnableSocietyOwnedVehicles  and PlayerData.job.name == 'taxi' then
    ...
    local coords = GetEntityCoords(GetPlayerPed(-1))
    if IsControlPressed(0, Keys['N+']) and PlayerData.job ~= nil and PlayerData.job.name == 'taxi' and (GetGameTimer() - GUI.Time) > 150 then
      if GetDistanceBetweenCoords(coords, 172.25, -26.50, 68.34, false) <= 35.0 then
        if OnJob then
          StopTaxiJob()
        else
          local playerPed = GetPlayerPed(-1)
        ...
        ESX.ShowNotification("Jesteś za daleko postoju aby przyjąć nowe zlecenie od dyspozytora!")
      end
    end
