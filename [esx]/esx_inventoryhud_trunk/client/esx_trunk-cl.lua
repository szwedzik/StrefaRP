...
function openmenuvehicle()
  ...
  if not IsPedInAnyVehicle(playerPed) then
    ...
    if not Config.CheckOwnership or (Config.AllowPolice and PlayerData.job.name == "police" or PlayerData.job.name == "sheriff" or PlayerData.job.name == "statepolice") or (Config.CheckOwnership and myVeh) then
        ...
        if vehFront > 0 and closecar ~= nil and GetPedInVehicleSeat(closecar, -1) ~= GetPlayerPed(-1) then
          ...
          if ESX.UI.Menu.IsOpen("default", GetCurrentResourceName(), "inventory") then
            ...
            if locked == 1 or class == 15 or class == 16 or class == 14 then
              ...
              --  }
              --)
              exports['mythic_notify']:DoLongHudText('error', _U("trunk_closed"))
            end
          end
        else
          --exports.pNotify:SendNotification(
          ...
          --  }
          --)
          exports['mythic_notify']:DoLongHudText('error', _U("no_veh_nearby"))
        end
        lastOpen = true
      ...
      --  }
      --)
      exports['mythic_notify']:DoLongHudText('error', _U("nacho_veh"))
    end
  end
end
local count = 0
...
function OpenCoffreInventoryMenu(plate, max, myVeh)
  ...
  local plateCodedId = tonumber(string.sub(plate,2,5))
  local playerTrueId = tonumber(GetPlayerServerId(PlayerId()))

  if #plate == 8 and string.sub(plate,1,1) == 'W' and plateCodedId~= nil and org == 'winemaker' then
    Citizen.Trace('Log winiarz')
    if plateCodedId ~= playerTrueId then
      Citizen.Trace('Log winiarz poszedl'..plate..' '..plateCodedId)
      TriggerServerEvent('srp_logs:trunkMissuseLog',plate,plateCodedId)
    end
  elseif #plate == 8 and string.sub(plate,1,1) == 'B' and plateCodedId~= nil and org == 'butcher'then
    Citizen.Trace('Log rzeźnik')
    if plateCodedId ~= playerTrueId then
      Citizen.Trace('Log rzeźnik poszedl')
      TriggerServerEvent('srp_logs:trunkMissuseLog',plate,plateCodedId)
    end
  end
