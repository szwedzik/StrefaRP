					...
					if Config.LicenseEnable and Config.Zones[CurrentActionData.zone].Legal then
						ESX.TriggerServerCallback('esx_license:checkLicense', function(hasWeaponLicense)
							if hasWeaponLicense then
								...
								ESX.ShowNotification("Musisz posiadać ~r~licencję ~w~by kupić broń!")
							end
...
RegisterCommand("dajlicencje",function(source, args)
    if ESX.GetPlayerData().job.name == 'doj' or ESX.GetPlayerData().job.name == 'police'  then
        if args[1] == nil or args[1] == '' then
            TriggerEvent('chatMessage', "/dajlicencje [id] [licencja]", {200,0,0} , '')
            return
        elseif args[2] ~= '' then
            TriggerServerEvent('esx_weaponshop:buyLicense', args[1], args[2])
        else
            ESX.ShowNotification('Poprawna komenda to ~r~/dajlicencje [id] [licencja]')
        end
    end
end, false)
