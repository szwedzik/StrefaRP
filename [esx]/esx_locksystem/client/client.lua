			...
			if(localVehId and localVehId ~= 0)then
				if(isInside) then
					...
					if(hasKey) then
						...
						TriggerEvent("ls:notify", "Zostawiasz kluczyki")
					else
						TriggerEvent("ls:notify", "Nie masz kluczków")
					end
				end
			end
...
function Notify(text, duration)
	if(Config.notification)then
		if(Config.notification == 1)then
			...
			Citizen.InvokeNative(0x1E6611149DB3DB6B, "CHAR_CARSITE", "CHAR_CARSITE", true, 1, text, '', duration)
			DrawNotification_4(false, true)
		elseif(Config.notification == 2)then
			TriggerEvent('chatMessage', text, {255, 255, 255}, '')
		else
			return
		end
	else
