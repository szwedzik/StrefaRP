...
AddEventHandler('esx_vehicleshop:setVehicleOwned', function (vehicleProps, model)
	...
	TriggerEvent('srp_logs:cardealerlog',vehicleProps.plate,1,_source)
	--TriggerClientEvent('esx:showNotification', _source,'wywolalo')--..'model '..GetDisplayNameFromVehicleModel(vehicleProps.model))
...
AddEventHandler('esx_vehicleshop:setVehicleOwnedPlayerId', function (playerId, vehicleProps, model)
	local xPlayer = ESX.GetPlayerFromId(playerId)
	TriggerEvent('srp_logs:cardealerlog',vehicleProps.plate,1,playerId)
	MySQL.Async.execute('INSERT INTO owned_vehicles (owner, plate, vehicle, modelname) VALUES (@owner, @plate, @vehicle, @modelname)',
	{
		['@owner']   = xPlayer.identifier,
		['@plate']   = vehicleProps.plate,
...
AddEventHandler('esx_vehicleshop:sellVehicle', function (vehicle)
	...
	local _source = source
	--TriggerEvent('srp_logs:cardealerlog',plate,0,_source)
...
AddEventHandler('esx_vehicleshop:returnProvider', function(vehicleModel)
	...
	MySQL.Async.fetchAll('SELECT * FROM cardealer_vehicles WHERE vehicle = @vehicle LIMIT 1', {
		...
		if result[1] then
			...
			TriggerClientEvent('esx:showNotification', _source,'Is it it?')
...
ESX.RegisterServerCallback('esx_vehicleshop:giveBackVehicle', function (source, cb, plate)
	MySQL.Async.fetchAll('SELECT * FROM rented_vehicles WHERE plate = @plate', {
		...
		if result[1] ~= nil then
			...
			--TriggerEvent('srp_logs:cardealerlog',plate,0,source)
					...
					if result[1] then -- does the owner match?
						...
						if vehicle.model == model then
							if vehicle.plate == plate then
								...
								TriggerEvent('srp_logs:cardealerlog','NIEZGODNE TABLICE '..plate,0,_source)
								print(('esx_vehicleshop: %s attempted to sell an vehicle with plate mismatch!'):format(xPlayer.identifier))
								cb(false)
							end
						else
							TriggerEvent('srp_logs:cardealerlog','NIEZGODNE MODELE '..plate,0,_source)
